...
namespace Scoreboard.Database
{
    public static class Repository
    {
        ...
        public static UserModel GetUserById(int id)
        {
            using (var cmd = new NpgsqlCommand(@"
                ...
                FROM Users WHERE id=@id and invalid = '0'", Conn))
            {
                cmd.Parameters.AddWithValue("@id", id);
                using (var dr = cmd.ExecuteReader())
                {
        ...
        public static UserModel GetUserByName(string username)
        {
            using (var cmd = new NpgsqlCommand(@"
                SELECT id, name, password, role_id, fullname, phone, email
                FROM Users WHERE LOWER(name)=LOWER(@u) and invalid = '0'", Conn))
            {
        ...
        {
            using (var cmd = new NpgsqlCommand(@"
                INSERT INTO Users (name, password, role_id, fullname, phone, email)
                VALUES (@n, @p, @r, @f, @ph, @e, '0')", Conn))
            {
        ...
        public static List<MatchModel> GetAllMatches()
        {
            ...
            string sql = @"
                    ...
                    LEFT JOIN Users u ON m.referee_id = u.id
                    LEFT JOIN Tournaments t ON m.tournament_id = t.id
                    LEFT JOIN MatcheClass ms ON t.match_class_id = ms.id
                    ORDER BY m.id;
                ";
        ...
        public static void AddMatch(MatchModel m)
        {
            ...
            using (var cmd = new NpgsqlCommand(sql, Conn))
            {
                ...
                cmd.Parameters.AddWithValue("@stt", m.Status ?? (object)DBNull.Value);
        ...
        public static MatchsetModel GetNextMatchDetail(string matchId, int userId)
        {
            ...
            string sql = @"
                ...
                WHERE ms.status = '0'
                  AND ms.match_id = @mid
                  AND (ms.referee_id = @uid OR @uid IS NULL)
        ...
        public static void AddMatchSet(MatchsetModel m)
        {
            ...
            string sql = @"
                ...
                (id, match_id, score1, score2, start, ""end"", time, note, status, ClassSets_id, referee_id)
                VALUES (@id, @mid, @s1, @s2, @st, @et, @t, @note, @stt, @cid, @rid);
            ";
            ...
            using (var cmd = new NpgsqlCommand(sql, Conn))
            {
                ...
                cmd.Parameters.AddWithValue("@t", m.Note);
        ...
        public static void DeleteMatchSet(int id)
        {
            using (var cmd = new NpgsqlCommand("DELETE FROM MatchSets WHERE id=@id", Conn))
            {
                cmd.Parameters.AddWithValue("@id", id);
                cmd.ExecuteNonQuery();
            }
        ...
        // MATCHCLASS
        // ====================================================
        public static List<MatcheClassModel> GetAllMatcheClasses()
        {
            var list = new List<MatcheClassModel>();

            using (var cmd = new NpgsqlCommand("SELECT id, name FROM MatcheClass ORDER BY id", Conn))
            using (var dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
                    list.Add(new MatcheClassModel
                    {
                        Id = dr.GetInt32(0),
                        Name = dr.GetString(1)
                    });
        ...
        public static void AddMatcheClass(MatcheClassModel m)
        {
            using (var cmd = new NpgsqlCommand("INSERT INTO MatcheClass(name) VALUES(@n)", PostgresHelper.SharedConn))
            {
                cmd.Parameters.AddWithValue("@n", m.Name);
                cmd.ExecuteNonQuery();
            }
        }
        public static void UpdateMatcheClass(MatcheClassModel m)
        {
            using (var cmd = new NpgsqlCommand("UPDATE MatcheClass SET name=@n WHERE id=@id", PostgresHelper.SharedConn))
            {
                cmd.Parameters.AddWithValue("@id", m.Id);
                cmd.Parameters.AddWithValue("@n", m.Name);
                cmd.ExecuteNonQuery();
        ...
        public static List<ClassSetsModel> GetAllClassSets()
        {
            ...
            using (var cmd = new NpgsqlCommand("SELECT id, name, Match_Class_Id FROM ClassSets ORDER BY id", PostgresHelper.SharedConn))
            using (var dr = cmd.ExecuteReader())
            {
                while (dr.Read())
                {
    ...
    // MODELS
    // ====================================================
    public class RoleModel { public int Id { get; set; } public string Name { get; set; } }
    public class ClassSetsModel
    {
